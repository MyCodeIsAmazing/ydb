...
_Аудитный лог_ — это поток записей, фиксирующих работу кластера {{ ydb-short-name }}. В отличие от диагностических логов, которые помогают в расследовании технических инцидентов и устранении неполадок, аудитный лог предоставляет данные значимые для безопасности. Он служит источником информации, отвечая на вопросы: кто что сделал, когда и откуда.
...
Секция [`audit_config`](#audit-log-configuration) в конфигурации кластера определяет, какие аудит-логи собираются, как они сериализуются и куда доставляются.
...
|| `program_text`           | MiniKQL-программа, отправленная с запросом. ||
|| `schema_changes`         | Описание изменений схемы, запрошенных в операции. ||
|| `table`                  | Полный путь таблицы. ||
|| `row_count`              | Количество строк, обработанных запросом bulk upsert. ||
