...
_Аудитный лог_ — это поток записей, который содержит информацию обо всех операциях над объектами {{ ydb-short-name }}. Аудитный лог фиксирует:
...
В отличие от диагностических логов, которые фиксируют детали реализации для устранения неполадок, аудитный лог сохраняет сведения об ответственности для мониторинга безопасности, проверки соответствия требованиям и расследования инцидентов.

Раздел кластерной конфигурации [`audit_config`](#audit-log-configuration) определяет, как сериализуются эти события и куда они доставляются. Настраивая этот раздел, вы решаете [какие аудитные логи](#log-classes) вам важны и [куда они должны писаться](#audit-log-destinations).
...
Аудитные события группируются в *классы логирования*, которые отражают основные категории операций. Вы можете включать или отключать запись для каждого класса в [конфигурации](#log-class-config) и при необходимости настраивать параметры отдельно для каждого класса. Доступные классы логирования:
...
|| `Ddl`                | Запросы на Data Definition Language (DDL). ||
|| `Dml`                | Запросы на Data Manipulation Language (DML). ||
...
|| [gRPC-сервисы](#grpc-proxy) </br>`grpc-proxy` | Внешние gRPC-запросы, обрабатываемые API {{ ydb-short-name }}. | Включите соответствующие [классы логирования](#log-class-config) и при необходимости [фазы логирования](#log-phases). ||
...
|| **Поля запроса**         | **>** ||
|| `query_text`             | Санитизированный текст YQL-запроса. ||
|| `prepared_query_id`      | Идентификатор подготовленного запроса. ||
|| `program_text`           | Поле `ExecuteTabletMiniKQL` в TabletService. ||
|| `schema_changes`         | Поле `ChangeTabletSchema` в TabletService. ||
|| `table`                  | Полный путь к таблице. ||
|| `row_count`              | Количество строк, обработанных `BulkUpsert`. ||
...
Ниже приведена простая конфигурация, сохраняющая аудитный лог в текстовом виде в файле с форматом `TXT`.
...
Пример конфигурации, которая отправляет аудитный лог в Unified Agent в формате `TXT` с меткой `audit`, а также выводит его в `stderr` в формате `JSON`. Настройки `Default` включают логирование всех классов в фазе `Completed`. Дополнительно `ClusterAdmin` настроен для логирования фазы `Received`, а `DatabaseAdmin` — для исключения событий от анонимных пользователей:
